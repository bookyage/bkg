	...
	class WPKQCG_Forms_QRCode
	{
		...
		private static function get_fields()
		{
			...
			$fields = array(
				...
				array(
					...
					'_label'			=> __('Use automatic current page URL as QR Code image clickable link.', WPKQCG_TEXT_DOMAIN),
					'_value_default'	=> 0
				),
				array(
					'_meta_type'		=> 'text', 
		...
		*/
		private static function validate_field($p_field_slug, $p_field_value, $p_field_value_default)
		{
			...
			{
				//	size is numeric
				$field_valid = (!empty($p_field_value) && is_numeric(intval($p_field_value))) ? intval($p_field_value) : $p_field_value_default;
