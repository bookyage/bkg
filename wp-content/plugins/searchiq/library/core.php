...
class siq_core{
	...
	public function addToSync($postIDs, $deleteUnsynced = true){
		...
		if( is_array($postIDs) && count($postIDs) > 0 ) {
			...
			if( !empty($query) ){
				 $query .= " ON DUPLICATE KEY UPDATE sync_time = '".$currentTime."' ";
			}		
			$wpdb->query($query);			
			$result["success"] = 1;
	...
	public function getSyncPostIds($status = 'd', $getCount = false, $limit = 0){
		...
		if($status == "d"){
			...
			$theQuery = "SELECT tbl1.id as post_id FROM $table_name tbl";
			$theQuery .=" RIGHT JOIN $post_table_name tbl1 ON tbl.post_id=tbl1.id";
